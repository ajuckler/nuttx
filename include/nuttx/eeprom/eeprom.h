...
#define EEPIOC_GEOMETRY     _EEPIOC(0x000)  /* Similar to BIOC_GEOMETRY:
                                             ...
                                             *      buffer. */
#define EEPIOC_PAGEERASE    _EEPIOC(0x001)  /* Erase a single page
                                             * IN:  Index of the page to be erased
                                             * OUT: None (ioctl return value provides
                                             *      success/failure indication). */
#define EEPIOC_SECTORERASE  _EEPIOC(0x002)  /* Erase a single sector, this behaves
                                             * like page erase on devices without
                                             * sectors
                                             * IN:  Index of the sector to be erased
                                             * OUT: None (ioctl return value provides
                                             *      success/failure indication). */
#define EEPIOC_CHIPERASE    _EEPIOC(0x003)  /* Wipe the entire EEPROM
                                             * IN:  None
                                             * OUT: None (ioctl return value provides
                                             *      success/failure indication). */
